  ...
  <GiPageLayout margin>
    <a-row justify="space-between" class="gi-row-tool">
      ...
      <a-space wrap>
        <a-input-group>
          ...
          <a-input v-model="queryParams.roleName" placeholder="输入角色名搜索" allow-clear style="width: 250px"></a-input>
        </a-input-group>
    ...
    <a-table class="gi-table" row-key="id" :data="roleList" :bordered="{ cell: true }" :loading="loading"
      ...
      <template #columns>
        ...
        <a-table-column title="角色编码" data-index="roleCode" :width="150"></a-table-column>
...
const onPerm = (item: T.ListItem) => {
  PermModalRef.value?.open({ id: item.id, title: item.roleName })
}
</script>
