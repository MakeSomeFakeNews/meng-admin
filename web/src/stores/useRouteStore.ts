...
import type { MenuTreeVO } from '@/apis/user'
 ...
 * ])
 */
const formatAsyncRoutes = (menus: MenuTreeVO[]) => {
  ...
  const routes = mapTree(menus, (item) => {
    ...
    return {
      ...
      meta: {
        hidden: item.visible === 0, // visible=0 表示隐藏
        keepAlive: false, // 默认不缓存
        title: item.title, // 路由标题，用于显示在菜单和标签页
        icon: item.icon, // 图标
        affix: false, // 默认不固定在标签页
        breadcrumb: true, // 默认显示在面包屑
        showInTabs: true, // 默认显示在标签页
        activeMenu: '', // 默认无激活菜单项
        alwaysShow: false // 默认不总是显示
      }
    }
  })
